...
// 강사 - 파일 업로드로 챕터 추가
export const createLectureByUpload = (courseId: number, title: string, file: File, preview?: boolean): Promise<Lecture> => {
...
// 강사 - 파일 재업로드로 챕터 수정
export const updateLectureByUpload = (courseId: number, lectureId: number, title: string, file: File, preview?: boolean): Promise<Lecture> => {
  const form = new FormData()
  form.append('title', title)
  form.append('file', file)
  form.append('preview', String(!!preview))
