...
import { getCourseReviews, addReview, getMyReview, updateReview, deleteReview, type Rating } from '../api/ratings'
import {
  getCourseQuestions,
  getQuestion,
  createQuestion,
...
export default function CourseDetailPage() {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (accessToken) {
          ...
          try {
            const myRating = await getMyReview(id)
            if (myRating) {
  ...
  async function handleRatingSubmit(e: React.FormEvent) {
    ...
    try {
      const newRating = await addReview(id, { score: myScore, comment: myComment })
  ...
  async function handleRatingUpdate(e: React.FormEvent) {
    ...
    setRatingLoading(true)
    try {
      const updated = await updateReview(id, myRatingId, { score: myScore, comment: myComment })
  ...
  async function handleDeleteRating(ratingId: number) {
    ...
    try {
      await deleteReview(id, ratingId)
