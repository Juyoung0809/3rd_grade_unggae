...
| 메서드 | 엔드포인트 | 인증 | 설명 |
|-------|-----------|------|------|
| POST | `/api/payments/prepare` | [AUTH] STUDENT | 결제 준비 (orderId 생성) |
| POST | `/api/payments/toss/confirm` | [AUTH] STUDENT | 토스페이먼츠 결제 검증 & 확정 |
...
### POST `/api/payments/prepare` Request Body:
```json
{ "courseId": 5 }
```
**Response:**
...
### POST `/api/payments/toss/confirm` Request Body:
```json
{
  "paymentKey": "5zJ4xY7m0kODnyRpQWGrZnN49wdvKm2x...",
  "orderId": "a1b2c3d4-...",
  "amount": 49000
}
```
...
### 토스페이먼츠
```
[Client] POST /api/payments/prepare -> orderId, amount 수령
[Client] 토스 JS SDK 호출 (clientKey, orderId, amount)
[토스 서버] 결제창 표시 -> 사용자 결제
[Client] successUrl 콜백 수신 (paymentKey, orderId, amount)
[Client] POST /api/payments/toss/confirm
[Server] 토스 API POST https://api.tosspayments.com/v1/payments/confirm 검증
[Server] 검증 성공 -> PAYMENT 저장, ENROLLMENT 생성
```